...
| `search_audit_guidelines` | 自然語言 + 科別 | 審查注意事項摘要 | v0.2 |
| `lookup_drug` | 學名 / 商品名 / 健保碼 | 健保藥品目錄 | v0.3 |
| `lookup_fee_code` | 代碼 / 名稱 / 類別 | 健保支付標準 | v0.4 |
| `lookup_audit_clauses_for_procedure` | 處置碼 + 科別 | 引用該處置的審查注意事項 + risk flags | v0.5 |
| `lookup_audit_clauses_for_specialty` | 科別 + 關鍵字 + risk flag | 該科審查注意事項瀏覽 | v0.5 |
| `lookup_major_illness` | 類別碼 / 關鍵字 | 重大傷病範圍項目（ICD-10 涵蓋、申請要求、有效期）| v0.5 |
| `check_icd_for_major_illness_eligibility` | ICD-10 碼 | 該診斷可能符合的重大傷病類別（反查）| v0.5 |
| `lookup_audit_indicator` | 指標碼 / 類別 / 科別 / 處置碼 | 分析審查不予支付指標（閾值、適用範圍）| v0.5 |
| `lookup_appeal_statistics_by_category` | 爭議類別 / 階段 | 按類別的彙總申復訊號（few / several / many）| v0.6 |
| `count_appeal_precedents_for_rejection_code` | 拒付碼 / 處置碼 | 該碼相關申復案量 + 申復方勝率訊號 | v0.6 |

涵蓋資料：健保核刪代碼、ICD-10 對應處置碼、健保支付標準、健保藥品目錄、藥品給付規定、審查注意事項條文（按處置/科別查）、分析審查不予支付指標、重大傷病範圍項目、申復統計訊號（彙總層級）、健保署官方 Wiki（9 大分類：審查 / 藥品特材 / 費用 / 計畫 / 服務 / 投保 / 表單 / 紀錄 / 行政）。
...
你只要在 Claude Desktop 的設定檔貼一段 10 行 JSON，重啟 App，就多了 17 個健保查詢工具。之後在 Claude 用自然語言問：
- 「0317A 核刪代碼什麼意思？」
- 「L30.9 皮膚科能申報哪些處置？」
...
**完全不會**。這 17 個 tool 只查公開健保代碼、處置、Wiki — **不傳送任何病患名字、病歷號、臨床內容**。你問 AI「0317A 什麼意思」時，MCP 只送「0317A」這個字串到 proxy，proxy 只查健保資料表。
